  ...
  return (
    <Router history={history}>
      <Route component={Application}>
        ...
        <Route path="/confirm-donation" component={GotVeggies} onEnter={requireAuth(store, 'confirm-donation')}/>
        <Route path="/donation-confirmed" component={DonationSubmitted}/>
        <Route path="/sorrynotthistime" component={GotNoVeggies} onEnter={requireAuth(store, 'sorrynotthistime')}/>
        ...
        <Route path="/admin" component={Admin} onEnter={requireAuth(store, 'admin')}/>
...
function requireAuth(store, redirectTo) {
  return (nextState, transition) => {
    ...
    if (tokens && !auth.loggedIn && !auth.loggingIn) {
      ...
      if (redirectTo) {
        transition.to('/login/' + redirectTo);
      } else {
        transition.to('/login');
      }
    }
