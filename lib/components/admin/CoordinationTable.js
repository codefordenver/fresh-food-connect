...
export default class CoordinationTable extends React.Component {
  ...
  _setSelectedLocations(id, event) {
    let sendEmail = event.target.checked
    this.setState({
      locationsToEmail: {
  ...
  _getRowData(location) {
    ...
    return {
      id: location.id,
      'send email': <Checkbox checked={::this._isLocationSelected(location)} onCheck={this._setSelectedLocations.bind(this, location.id)}/>,
      address,
      city,
      state,
      zipcode,
  ...
  _isLocationSelected(location) {
    return this.state.locationsToEmail[location.id]
  }
