...
export function renderRoutes(history, store) {
  return (
    <Router history={history}>
      ...
      <Route path="/verify-email" onEnter={verifyEmailToken(store)}/>
...
function verifyEmailToken(store) {
  return (nextState, transition) => {
    const verifyToken = getQueryVariable('confirmation_token');
    store.dispatch(verifyEmail(verifyToken));
